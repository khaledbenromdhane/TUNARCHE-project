...
class EvenementController extends AbstractController
{
    ...
    public function index(Request $request, EvenementRepository $repo, ParticipationRepository $participationRepo): Response
    {
        ...
        // Show only current user's participations (not all participations)
        $participationsByEvent = [];
        $user = $this->getUser();
        if ($user) {
            foreach ($evenements as $evt) {
                $userParts = $participationRepo->findByUserAndEvent($user->getId(), $evt->getId());
                if (!empty($userParts)) {
                    $participationsByEvent[$evt->getId()] = $userParts;
                }
            }
        }
    ...
    public function participationUpdate(int $id, Request $request, ParticipationRepository $participationRepo, ManagerRegistry $m, ParticipationService $service, EvenementRepository $evenementRepo): Response
    {
        ...
        // Only allow editing own participations
        if ($participation->getUser() !== $this->getUser()) {
            if ($isAjax) {
                return $this->json(['success' => false, 'errors' => ['general' => 'Vous ne pouvez modifier que vos propres participations.']]);
            }
    ...
    public function search(Request $request, EvenementRepository $repo, ParticipationRepository $participationRepo): JsonResponse
    {
        ...
        $user = $this->getUser();
        foreach ($evenements as $evt) {
            // Only include current user's participations
            $participations = $user
                ? $participationRepo->findByUserAndEvent($user->getId(), $evt->getId())
                : [];
            $partData = [];
